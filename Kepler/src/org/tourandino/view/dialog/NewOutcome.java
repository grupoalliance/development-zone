...
public class NewOutcome extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public NewOutcome(JFrame parent, boolean modal) {
		super(parent, modal);
	...
	public NewOutcome(JDialog parent, boolean modal) {
		super(parent, modal);
		this.cashMovementController = new CashMovementController();
		this.errorLogController = new ErrorLogController();
		initializeComponents();
	...
	private void initializeComponents(){
		...
		lblConcepto.setBounds(22, 104, 102, 15);
		...
		btnAccept.setIcon(new ImageIcon(NewOutcome.class.getResource("/ar/com/tourandino/resources/accept.png")));
		...
		btnCancel.setIcon(new ImageIcon(NewOutcome.class.getResource("/ar/com/tourandino/resources/cancel.png")));
			...
			if(!txtDescr.getText().isEmpty()){
				if(dailyCashRecordController.dailyRecordExist()){
					if(!dailyCashRecordController.DailyRecordIsClosed()){
						try{
							if(cashMovementController.create(0 , Float.parseFloat(txtTotal.getText()), cmbCategory.getSelectedItem().toString()+", "+ txtDescr.getText()) > 0){
