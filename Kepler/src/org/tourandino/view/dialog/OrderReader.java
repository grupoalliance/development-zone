...
import org.tourandino.controller.BusPassengerController;
import org.tourandino.controller.ErrorLogController;
import org.tourandino.controller.FlightPassengerController;
import org.tourandino.controller.HotelPassengerController;
import org.tourandino.controller.InsurancePassengerController;
import org.tourandino.controller.OrderController;
import org.tourandino.controller.PackagePassengerController;
...
public class OrderReader extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public OrderReader(JDialog parent, boolean modal, OrderController orderController, Integer id) {
		super(parent, modal);
	...
	private void initializeComponents(Integer id){
		...
		btnSalir.setIcon(new ImageIcon(OrderReader.class.getResource("/ar/com/tourandino/resources/door.png")));
	...
	private void loadTable(JTable table, Integer id, String descr){
		if(descr.contains("AEREO")){
			table.setModel(flightPassengerController.readByOrder(id));
			table.removeColumn(table.getColumnModel().getColumn(0));
		}
		if(descr.contains("COLECTIVO")){
			table.setModel(busPassengerController.readByOrder(id));
			table.removeColumn(table.getColumnModel().getColumn(0));
		}
		if(descr.contains("HOTEL")){
			table.setModel(hotelPassengerController.readByOrder(id));
			table.removeColumn(table.getColumnModel().getColumn(0));
		}
		if(descr.contains("PAQUETE")){
			table.setModel(packagePassengerController.readByOrder(id));
			table.removeColumn(table.getColumnModel().getColumn(0));
		}
		if(descr.contains("SEGURO")){
			table.setModel(insurancePassengerController.readByOrder(id));
			table.removeColumn(table.getColumnModel().getColumn(0));
		}
	}
	...
	private void load(Integer id){
		...
		try{
			...
			loadTable(tableCashMovements, Integer.parseInt(item[0].toString()), item[9].toString());
		}
